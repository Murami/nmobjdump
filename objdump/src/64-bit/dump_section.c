...
static void		print_raw_bytes64(t_dump_infos* dump_infos)
{
  unsigned int	i;
  int		cpy_ndword;
				       ...
				       char* section_name, Elf64_Ehdr* elf)
{
  ...
  if (gethost32(section->sh_size, elf) == 0 ||
    ...
    return (0);
  if (!fmapaccess(filemap, gethost64(section->sh_offset, elf),
		  gethost32(section->sh_size, elf)))
    {
      fprintf(stderr,
  ...
  while (dump_infos.offset < gethost32(section->sh_size, elf))
    {
      dump_infos.ndword = 0;
      dump_section_content_dword64(filemap, section, &dump_infos, elf);
      printf("\n");
    }
  return (0);
}
